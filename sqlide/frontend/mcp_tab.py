...
The tab opens popped out into a window of its own (the main window's
generic tab pop-out — see frontend/window.py): a running server is a
background process the user wants to keep an eye on independent of
whatever tab layout they're working in. It is an ordinary tab, so it
can be moved back into the tab bar like any other.
"""
