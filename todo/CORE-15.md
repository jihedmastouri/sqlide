...
### Acceptance criteria (finalised from the spike)

- [x] A monitoring view per connection, reachable from the connection node.
- [x] Live activity: current sessions/processes with user, database, state,
      duration and query, refreshable and sortable.
- [x] Ability to cancel or kill a session, with confirmation and the privileges
      checked first.
- [x] Throughput/health charts over a rolling window (connections, transactions
      or queries per second, cache hit ratio, locks/waits).
- [x] Storage: database and largest-table sizes.
- [x] Polling interval is configurable and pausable; monitoring stops when the
      view is closed.
- [x] Missing privileges or extensions produce an explicit "not available because
